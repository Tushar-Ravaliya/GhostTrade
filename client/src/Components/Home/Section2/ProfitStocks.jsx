    ...
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2.5">
          ...
          <h2 className="text-xl md:text-2xl font-bold text-text-primary">Top Gainers</h2>
        </div>
        <Link
          to="/market"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading ? (
          ...
          data.map((stock) => {
            ...
            return (
              <Link
                ...
                className="group"
              >
                <div className="bg-white rounded-2xl p-5 border border-border hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5 transition-all duration-300 hover:-translate-y-1">
                  {/* Top Row: Logo + Badge */}
                      ...
                      {logoUrl ? (
                        <img
                          ...
                          onError={(e) => {
                            ...
                            e.target.parentNode.innerHTML = `<span class="text-primary font-bold text-lg">${stock.symbol.charAt(0)}</span>`;
                          }}
                        />
                      ) : (
                        <span className="text-primary font-bold text-lg">{stock.symbol.charAt(0)}</span>
                      )}
                    </div>
                    <span className="px-2.5 py-1 bg-primary-50 text-primary text-xs font-bold rounded-full">
                      +{change}%
                    </span>
                  </div>
          ...
          <p className="text-text-muted col-span-full text-center py-8">No gainers data available</p>
        )}
      </div>
    </section>
  );
